...
| A5 | **Behavioral (significant) — INTERIM** | Coder marks initial prompt + last cacheable context with `CacheControl` → Anthropic prompt caching | **No cache control**: the neutral message model has no cache hint; the Anthropic adapter emits plain text blocks only → Maestro prompt caching is **silently dropped** (cost/latency regression) | App-shaped `CacheControl` must not enter the neutral model without an ADR-level design | **Blocks cut-over.** Tracked: ADR-0008 / a provider-neutral cache-breakpoint hint (task #12). This row resolves when that lands; until then the regression is real and recorded here, not silent |
