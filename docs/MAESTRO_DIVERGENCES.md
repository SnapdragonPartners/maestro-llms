...
| M4 | **Behavioral (significant)** | Open circuit emits `ServiceUnavailableError` wired to drive agent **SUSPEND**; breaker counts generic failures | Open circuit emits a distinct **non-retryable** `*middleware.CircuitOpenError` (no app coupling); only `llms.Retryable` failures trip it (auth/bad_request are neutral). See ADR-0005 | Fail-fast: retry is outside the circuit, so the open error must not be retryable; app SUSPEND policy is not the toolkit's concern | Maestro maps `*CircuitOpenError` to its own suspend/backoff at the app boundary; confirm misconfig (auth) no longer trips the breaker is acceptable |
